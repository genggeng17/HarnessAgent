...
def test_budget_check_within_budget(tmp_path: Path) -> None:
    service = service_at(tmp_path)
    service.add_expense(spent_on="2026-08-01", category="餐饮", amount="30.00")
    result = service.budget_check(month="2026-08", limit="100.00")
    assert result["month"] == "2026-08"
    assert result["limit"] == Decimal("100.00")
    assert result["spent"] == Decimal("30.00")
    assert result["remaining"] == Decimal("70.00")
    assert result["over_budget"] is False
    assert result["over_amount"] == Decimal("0")


def test_budget_check_over_budget(tmp_path: Path) -> None:
    service = service_at(tmp_path)
    service.add_expense(spent_on="2026-08-01", category="餐饮", amount="80.00")
    service.add_expense(spent_on="2026-08-02", category="交通", amount="30.00")
    result = service.budget_check(month="2026-08", limit="100.00")
    assert result["spent"] == Decimal("110.00")
    assert result["remaining"] == Decimal("-10.00")
    assert result["over_budget"] is True
    assert result["over_amount"] == Decimal("10.00")


def test_budget_check_exact_budget(tmp_path: Path) -> None:
    service = service_at(tmp_path)
    service.add_expense(spent_on="2026-08-01", category="餐饮", amount="100.00")
    result = service.budget_check(month="2026-08", limit="100.00")
    assert result["remaining"] == Decimal("0")
    assert result["over_budget"] is False


def test_budget_check_invalid_month(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="YYYY-MM"):
        service_at(tmp_path).budget_check(month="2026-8", limit="100.00")


def test_budget_check_invalid_limit(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="预算金额格式无效"):
        service_at(tmp_path).budget_check(month="2026-08", limit="abc")


def test_budget_check_non_positive_limit(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="预算金额必须大于零"):
        service_at(tmp_path).budget_check(month="2026-08", limit="0")
    with pytest.raises(ValueError, match="预算金额必须大于零"):
        service_at(tmp_path).budget_check(month="2026-08", limit="-50")
