...
def build_parser() -> argparse.ArgumentParser:
    ...
    budget = commands.add_parser("budget", help="检查月度预算")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "add":
            ...
            result = service.budget_check(month=args.month, limit=args.limit)
            print(f"月份：{result['month']}")
            print(f"预算：{result['limit']}")
            print(f"支出：{result['spent']}")
            print(f"剩余：{result['remaining']}")
            if result['over_budget']:
                print(f"超支：{result['over_amount']}")
