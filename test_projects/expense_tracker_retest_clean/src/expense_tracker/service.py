...
class ExpenseService:
    ...
    def check_budget(self, *, month: str, limit: str) -> "BudgetCheck":
        """检查指定月份的预算执行情况。"""

        from expense_tracker.models import BudgetCheck

        try:
            parsed_limit = Decimal(limit)
        except InvalidOperation as exc:
        ...
        remaining = parsed_limit - spent
        return BudgetCheck(month=month, limit=parsed_limit, spent=spent, remaining=remaining)
